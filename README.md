...
**숙제 기록·학습 기록** 메뉴는 선생님들이 이미 쓰시던 **구글 스프레드시트**(숙제 기록/학습 기록
탭)를 직접 읽고 씁니다(Firestore 에는 저장하지 않습니다). 둘 다 Google Cloud **서비스 계정**으로
서버(Vercel Serverless Functions, `api/`)에서 동작하므로 선생님들은 별도 로그인/연결 없이 그대로
사용하면 됩니다.
...
### 3) 숙제·학습 기록 스프레드시트 확인

선생님들이 이미 쓰고 계신 스프레드시트의 **"숙제 기록"**, **"학습 기록"** 탭을 그대로
사용합니다(새 탭을 만들 필요 없음). 두 탭 모두 1행 제목, 2행 부제, 3행 공백, **4행이 헤더**,
5행부터 데이터가 이어지는 기존 구조를 그대로 유지하고, 각 탭 헤더 행의 **맨 끝 열에 "기록ID"
컬럼을 하나만 추가**합니다(수정·삭제 시 행을 정확히 찾기 위한 용도로, 이 앱만 사용합니다).

**숙제 기록** 탭 — 기존 A~L 열(수업일/반 구분/학생명/등록 회차/수업 내용/숙제 작성란/제출
상태/확인 상태/피드백/재확인/다음 확인일/메모) 뒤 **M 열에 "기록ID"** 추가:

| A | B | C | D | E | F | G | H | I | J | K | L | M |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 수업일 | 반 구분 | 학생명 | 등록 회차 | 수업 내용 | 숙제 작성란 | 제출 상태 | 확인 상태 | 피드백 | 재확인 | 다음 확인일 | 메모 | 기록ID |

**학습 기록** 탭 — 기존 A~N 열(수업일/반 구분/학생명/등록 회차/출결/학습 주제/학습 목표/배운
내용/이해도/참여도/잘한 점/보완할 점/다음 수업 계획/선생님 메모) 뒤 **O 열에 "기록ID"** 추가:

| A | B | C | D | E | F | G | H | I | J | K | L | M | N | O |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 수업일 | 반 구분 | 학생명 | 등록 회차 | 출결 | 학습 주제 | 학습 목표 | 배운 내용 | 이해도 | 참여도 | 잘한 점 | 보완할 점 | 다음 수업 계획 | 선생님 메모 | 기록ID |

앱은 이 헤더 텍스트를 읽지 않고 열 순서만 고정 매핑으로 사용하므로, "기록ID" 라벨을 직접
입력하지 않아도 기능은 정상 동작합니다(가독성을 위한 선택 사항).

시트 우측 상단 **공유** 버튼으로 서비스 계정 이메일을 **편집자** 권한으로 추가합니다(이미
공유되어 있다면 생략). 시트 URL의 `/d/` 와 `/edit` 사이 문자열이 스프레드시트 ID 입니다.

> 앱이 새 행을 추가한 뒤, 시트의 파란 "표(Table)" 서식 범위가 새 열/행까지 자동으로 늘어나
> 있지 않으면 표 우측 하단 핸들을 드래그해 범위를 넓혀 주세요 — 표시상의 문제일 뿐 데이터
> 자체는 정상 저장됩니다.
...
| `GOOGLE_SHEETS_SPREADSHEET_ID` | 3단계에서 확인한 스프레드시트 ID |
| `GOOGLE_SHEETS_HOMEWORK_SHEET_NAME` | "숙제 기록" 탭 이름 (기본값 `숙제 기록`) |
| `GOOGLE_SHEETS_LEARNING_SHEET_NAME` | "학습 기록" 탭 이름 (기본값 `학습 기록`) |
| `FIREBASE_PROJECT_ID` | `/api/*` 의 실제 인가 경계. `firebase-config.js` 의 `projectId` 와 동일한 값이어야 함 |
| `INTERNAL_API_TOKEN` | `/api/*` 보조 봇 필터용 임의의 문자열 (예: `openssl rand -hex 32`) |

`/api/*` 는 요청마다 **로그인한 사용자의 Firebase ID 토큰**을 서버에서 직접 검증합니다
(`FIREBASE_PROJECT_ID` 필요). 프론트엔드는 이미 로그인된 Firebase 세션에서 토큰을 꺼내
`Authorization: Bearer …` 헤더로 자동으로 실어 보내므로 추가로 할 일은 없습니다 — 다만
`FIREBASE_PROJECT_ID` 를 반드시 등록해야 `/api/*` 가 401 없이 정상 동작합니다.

`INTERNAL_API_TOKEN` 을 설정했다면, [public/api-config.js](./public/api-config.js) 파일의
`window.HAOTING_API_TOKEN` 값도 **동일하게** 채워 넣으세요. 이 파일은 `firebase-config.js` 와
마찬가지로 커밋되는 파일이라 브라우저에 그대로 노출됩니다 — 그래서 이 토큰은 절대
"진짜 비밀"이 될 수 없고, 실제 인가는 위 `FIREBASE_PROJECT_ID` 기반 로그인 검증이 담당합니다.
`INTERNAL_API_TOKEN` 은 URL만 아는 임의의 요청/봇을 걸러내는 보조 장치일 뿐입니다.
...
- **캘린더 (양방향)**: 제목 형식은 `[담당강사] 학생이름 N회차` 로 고정입니다
  (예: `[박환희] 홍길동 3회차`). **캘린더에 직접 일정을 넣을 때도 이 형식,
  특히 "N회차" 를 반드시 지켜 주세요** — 회차 번호가 어느 학생의 몇 번째 수업인지를
  정확히 결정하는 유일한 단서입니다.
  - **사이트 → 캘린더**: 학생 상세의 회차별 날짜·시작·종료 시간을 저장할 때마다 위 형식으로
    자동 생성/수정됩니다. 날짜/시간을 모두 지우면 이벤트도 삭제됩니다.
  - **캘린더 → 사이트**: 로그인해서 사이트를 열 때마다 마지막 확인 시점 이후 캘린더에서
    바뀐 내용을 가져와 반영합니다(실시간 아님, 페이지 로드 시점 기준 폴링).
    - 이미 사이트가 만든 이벤트(연결된 회차가 있음)는 시간 변경·삭제가 그대로 반영되고,
      삭제된 이벤트는 해당 회차의 날짜/시간이 비워집니다.
    - **캘린더에서 직접 새로 만든** 일정은 제목을 파싱해 학생과 회차를 찾습니다. 다음 중
      하나라도 해당하면 반영하지 않고 **학생 관리 화면 상단에 실패 안내 배너**로 알려줍니다
      (제목 원문·사유 표시, "닫기"로 확인 처리):
      - 제목 형식이 다르거나 회차 번호가 없음
      - 이름이 일치하는 학생이 없거나 동명이인이 여러 명 있음
      - N회차가 등록 회차 수를 초과함
      - 그 회차가 이미 다른 캘린더 일정과 연결되어 있음(중복 입력 의심)
      - 하루 종일 일정 등 시작/종료 시간이 없는 일정
    - 애매한 경우 추측해서 채우지 않고 항상 실패 목록에 남기는 것이 원칙입니다 — 잘못된
      학생/회차에 조용히 반영되는 것보다 안전합니다.
- **숙제 기록 / 학습 기록**: 스프레드시트가 원본이라 Firestore 실시간 반영과 달리 자동
  새로고침되지 않습니다. 화면의 **새로고침** 버튼으로 최신 상태를 다시 불러오세요. 삭제는 해당
  행의 값을 모두 비우는 방식으로 처리됩니다(행 자체는 남지만 목록에는 더 이상 보이지 않음).
