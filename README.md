...
두 명의 선생님이 각자 계정으로 로그인해 시스템에 접근합니다.

| 선생님 | 아이디 | 비밀번호 |
| --- | --- | --- |
| 박환희 선생님 | `admin1` | `haoting1234` |
| 김정화 선생님 | `admin2` | `haoting1234` |

- 로그인 세션은 브라우저 Local Storage(`haoting:session:v1`)에 저장되어, 새로고침해도 유지됩니다. 로그아웃 시 즉시 삭제됩니다.
- 사이드바 하단·헤더 우측에서 현재 로그인한 선생님 정보를 확인할 수 있고, 같은 위치의 로그아웃 버튼으로 즉시 로그아웃됩니다.
- 비밀번호는 `app.js` 의 `ACCOUNTS` 상수에서 변경할 수 있습니다.

> ⚠️ 본 시스템의 로그인은 클라이언트 사이드 "소프트 게이트" 입니다. 누구나 코드를 열어 계정을 확인할 수 있으므로, 실제 보안이 필요한 경우 Firebase Authentication 도입을 권장합니다.
...
3. `admin1` / `admin2` 계정으로 로그인하면 학생 목록을 볼 수 있습니다.
